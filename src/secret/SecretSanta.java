...
public class SecretSanta {
     ...
     */
    private static final int COUNT_RETRY = 2;
     ...
     */
    private static void printInTheConsole(String text) {
     ...
     */
    private static FamilyMembers[] createParticipants(Scanner console) {
        System.out.println();
     ...
     */
    private static void listParticipants(FamilyMembers[] familyMembers) {
        System.out.println();
    ...
    private static void assignRecipients(FamilyMembers[] familyMembers,
            Random random) {
        System.out.println();
        ...
        while (!allHaveMatches) {
            ...
            for (int i = 0; i < familyMembers.length; i++) {
                ...
                try {
                    data1 = secret.DataReader.readText(familyMembers[i].getName() + "'s History.txt");
                    System.out.println("Previous santa for " + familyMembers[i].getName() + " was : " + data1);
                    ...
                    while (!person.hasRecipient() &&
                            retryCount < familyMembers.length * COUNT_RETRY) {
                        ...
                        if (num != i && !familyMembers[num].hasSender()) {
                            ...
                            for (int s = 0; s < number; s++) {
                                flag = !familyMembers[num].equals(newArrayList.get(s));
                            }
                            if (flag) {
                                ...
                                System.out.println(x + "=x" + familyMembers[x].getName());
                        ...
                        if (retryCount > familyMembers.length * COUNT_RETRY) {
    ...
    private static int findIndex(FamilyMembers[] familyMembers, ArrayList arrayList, int index) {
        ...
        int i = 0;
        while (i < len) {
            if (familyMembers[i].getName() == arrayList.get(index)) {
                return i;
            } else {
                i = i + 1;
            }
     ...
     */
    private static boolean allHaveMatches(FamilyMembers[] familyMembers) {
     ...
     * @param familyMembers the desired list of FamilyMembers objects
     */
    private static void resetMatches(FamilyMembers[] familyMembers) {
        for (FamilyMembers person : familyMembers) {
     ...
     * @param familyMembers the desired list of FamilyMembers objects
     */
    private static void printAll(FamilyMembers[] familyMembers) {
        System.out.println();
     ...
     * @param familyMembers the desired list of FamilyMembers objects
     */
    private static void saveAll(FamilyMembers[] familyMembers) {
        System.out.println();
        printInTheConsole("SAVE ALL PARTICIPANTS NAMES AND THEIR PAIRINGS TO A PROPERTIES FILE");
        String history;
        if (allHaveMatches(familyMembers)) {
            for (FamilyMembers person : familyMembers) {
                history = person.getRecipientName() + "'s_History" + ".txt";
                try {
                    secret.DataReader.readText(history);
                    secret.DataReader.appendStrToFile(history, person.getName());
                } catch (Exception ee) {
                    secret.DataReader.appendStrToFile(history, person.getName());
                }
            }
